...
def get_server_environments(host):
  ...
  The idea is that a client can talk to a server in either it's own environment
...
def get_nodes_with_package(package, domain=None, environments=('production',)):
  ...
  c.execute(
    'SELECT host.name, network.name, package.option, e.value FROM host, network, package '
    'LEFT OUTER JOIN option e ON e.node_id = host.node_id AND e.name = "env" '
    'WHERE package.node_id = host.node_id '
    'AND network.node_id = host.network_id AND package.name = ?', (package, ))
  packages = set(c.fetchall())
  conn.close()

  # Filter domain
  if domain:
    packages = [x for x in packages if x[1].startswith(domain.upper() + '@')]

  # Replace missing environment with 'production'
  if environments:
    packages = [tuple(x[:3]) + (x[3] if x[3] else 'production',) for x in packages]
    # Filter environment
...
def get_servers_for_node(package, host, use_domain=True, use_environments=True):
...
def get_networks_with_flag(flag, domain=None, value=None):
  ...
  c.execute(
    ...
    'ipv6_txt, ipv6_netmask_txt, ipv6_gateway_txt, option.value FROM network, '
    'option WHERE option.node_id = network.node_id '
    'AND option.name = ?', (flag, ))
  raw_data = set(c.fetchall())
  conn.close()

  # Filter domain
  if domain:
    raw_data = [x for x in raw_data if x[0].startswith(domain.upper() + '@')]
...
def get_networks_with_name(name):
  ...
  c.execute('SELECT ipv4_txt, ipv6_txt FROM network WHERE name = ?', (name, ))
  networks = c.fetchone()
  conn.close()
  return networks
