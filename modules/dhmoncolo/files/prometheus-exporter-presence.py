...
res = c.execute("""
  SELECT host.name, layer.value, os.value FROM host
        LEFT OUTER JOIN option AS layer ON host.node_id = layer.node_id AND layer.name = 'layer'
        LEFT OUTER JOIN option AS os    ON host.node_id = os.node_id    AND os.name = 'os'
