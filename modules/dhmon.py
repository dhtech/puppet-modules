...
def generate_backend(host, local_services):
  ...
  for package, spec in manifest['packages'].iteritems():
    if spec is None or not 'monitor' in spec:
      continue
    ...
    for url_id, url_str in urls.iteritems():
      ...
      targets = []
      for target in sorted(lib.get_nodes_with_package(package, domain).keys()):
  ...
  # icmp = blackbox('icmp', '127.0.0.1:1234', all_nodes, {'modules': ['icmp']})
